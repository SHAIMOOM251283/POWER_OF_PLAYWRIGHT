...
class Extraction:
    def __init__(self):
        self.query = "Chemistry"
        self.total_pages = 47  # Adjust to the number of pages needed
        self.concurrent_pages = 20  # Number of pages to scrape in parallel
        self.all_books = []  # Store all data in memory and save once at the end

    async def search_open_library(self, page, page_num):
        """Scrape a single page and return book details with retry."""
        print(f"Scraping page {page_num}...")
        books_data = []
        url = f"https://openlibrary.org/search?subject={self.query}&page={page_num}"
        ...
        return books_data  # Return whatever was collected (may be empty if all retries failed)
    ...
    async def extract(self):
        ...
        async with async_playwright() as p:
            ...
            # Save data to CSV once all pages are scraped
            csv_filename = os.path.join(data_folder, f"{self.query}.csv")
